...
function tooLong(input) {
    return input.length > MAX_INPUT_LENGHT
}
    ...
    if (firstNumber.length > 0 && secondNumber.length > 0 && operation) {
        switch (operation) {
            case OPERATION_DIVISION:
                firstNumber = (parseFloat(firstNumber) / parseFloat(secondNumber)).toString() //parse float to change from string to number
                break;
