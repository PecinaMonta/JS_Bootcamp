 ...
 */
function sayHello(isEvening) { // curly brackets
    if (isEvening){
        return 'Good evening'
    } else if (!isEvening) {
        return 'Good day'
 ...
 */
 function isEven(n) {
        return n % 2 === 0 // to check if number is even  // equal modulus  even
}
...
function chooseLongestWord(word, otherWord) {
    if (word.length === otherWord.length) {
        ...
        return word
    } else (word.length < otherWord.lenght) {
        return otherWord
    }
...
function getLastWordFromText(text) {
        ...
        let lastWord = split[split.length - 1].toUperCase()
        return lastWord
}
...
function addressFormally(fullName, isMarried, isFemale) {
    ...
    let firstCharacterOfLastName = nameSplit[1][0] // to get C
 ...
 */
function getOddNumbers(numbers) { // Modulo operator
    let oddNumbers = [] // empty array
    for (nr of numbers) { 
        if (nr % 2 !== 0) { // the number is even ... !=== not zero, its odd number!!
            odd.Numbers.push(nr)
        }  
    }
    return oddNumbers
...
function reverseText(text) {
    let reversed = ''
    for (let i = text.length - 1; i >= 0; i--){ // 3 -1 ir [2]
        //console.log(text[i])
        reversed += text[i]
    }
    return reversed
