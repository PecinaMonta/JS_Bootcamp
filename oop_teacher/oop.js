...
class Violin extends MusicIntruments {
...
class Piano extends MusicIntruments {
    constructor(size, color, type) {
        super(size, color)
        this.type = type
    }
...
class Orchestra {
    ...
    playTogether() {
        for (let musician of this.musicians) { //for of loop type
            musician.perform()
        }
    }
}
...
class MusicalEnterprise {
    ...
    makePerform(nameOfOrchestra) {
       ...
       // for (let orchestra of this.orchestras) {
        //    if (orchestra.name === nameOfOrchestra) {
        //        orchestra.playTogether()
        //    }
       // }
    }
...
let firstViolin = new Violin('medium', 'red', 'wood')
...
orchestra.addMusician(new Musician(new Violin('medium', 'green', 'metal'), 'Paul'))
...
myMap.set('john.wick@gmail.com', 'John Wick')  // set is a method
myMap.set('ulfrica.zerede@yandex.ru', 'Ulfrica Zerede')

let johnWick = myMap.get['gmailjohn.wick@gmail.com']
...
let newMap = new Map()
newMap.set(123, new Person('Janis', 'Esjkg', 50))
